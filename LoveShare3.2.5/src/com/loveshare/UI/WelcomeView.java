...
public class WelcomeView extends FrameLayout {
	...
	//****************************ä»¥ä?ä¸ºæ???å¤???å·±å??ä¹?????ï¿?
	private boolean isShowInput; //è¾??¥æ?å¼¹å?ºå?¤ï??
...
//		invalidate();
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int i = 0; i < count; i++) {
			...
			// å¦???ä¸???clickable??ä¸??½æ?£å¸¸??è°?onInterceptTouchEvent?¹æ?
	...
	private boolean find(View view, boolean isLeft, MotionEvent event) {
		if (view instanceof ViewGroup) {
			...
			for (int i = 0; i < parent.getChildCount(); i++) {
				...
				if (child instanceof WelcomeView) {
					WelcomeView ws = (WelcomeView) child;
...
//			QLog.i("System.out", getTag()+" find "+mDestnationScreen+" "+find+" left:"+left+" "+ev.getAction());
		...
		case MotionEvent.ACTION_MOVE:
			...
			if ((xMoved || yMoved) && tan < TAN30) {// å·??³æ??¨è?åº?ï¿?ï¿½ï¿½ï¿?0ï¿?
		...
		case MotionEvent.ACTION_MOVE:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if(deltaX > 0 &&!mAlloweffect && mCurrentScreen == getChildCount()-1){
					//è¿?ä¸??¤æ???¨ä?è§£å?³æ??°ï??ï¿½ï¿½ï¿?ï¿½ï¿½?¾æ?¶ï????¨å°±è¿??¥ä¸»ç¨?åº?äº?
